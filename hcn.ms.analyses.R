...
### -------------------------------------------------------------------------------------------
### -------------------------------------------------------------------------------------------
#### ------------------- Figure 3 Method Calibration Experiment - Desiccator vs Cup
...
### ------------------------------------------------------------------------
### ------------------------------------------------------------------------
#### ----------- APPENDIX S12 - Heliconius doris HCN Induction Figure  
...
### -------------------------------------------------------------------------------------------
### -------------------------------------------------------------------------------------------
#### ----- FIGURE S1 - Experimental  Validation/Control
# Spectrophotometer Comparison of Intra-Leaf Variation 
...
### ------------------------------------------------------------------------
### ------------------------------------------------------------------------
####  ----------- TABLE 1 - Comparison with Engler & Gilbert (2007) colorimetric CN data 
eng=read.csv("table3.csv", header=T)
head(eng,10)
eng2<-eng[-c(1,26,8,10),] # subset to remove unused rows
...
colnames(eng2)
diff <- eng$converted.published - eng$calibrated.plastic.cup.mean

#Then conduct a one-sample t–test to compare the mean difference to 0 { H(o) theoretical mean}. 
library(ggpubr)
t.test(diff, mu = 0, alternative = "two.sided") 
# t = -1.2906, df = 7, p-value = 0.2378 ---> H(a): true mean is not equal to 0
# THIS IS ANOTHER WAY TO DO THE SAME TEST:
t.test(eng$converted.published, eng$calibrated.plastic.cup.mean,paired=TRUE,conf.level=0.95)
# t = -1.2906, df = 7, p-value = 0.2378
